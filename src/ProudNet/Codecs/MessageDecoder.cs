...
namespace ProudNet.Codecs
{
    ...
    {
        private static readonly NLog.Logger Logger = NLog.LogManager.GetLogger("MessageDecoder");
        ...
        protected override void Decode(IChannelHandlerContext context, IByteBuffer message, List<object> output)
        {
            using (var r = new ReadOnlyByteBufferStream(message, false).ToBinaryReader(false))
            {
                ...
                if (factory == null)
                {
                    ...
                    Console.ResetColor();
                    Logger.Error($"opcode desconocido [{opCode}] " + message.ToArray().ToHexString());
                    return;
#else
                ...
                {
                    Logger.Error(ex, $"fallo deserializando el opcode [{opCode}]");
