...
namespace ProudNet.Codecs
{
    internal class CoreMessageDecoder : MessageToMessageDecoder<IByteBuffer>
    {
        ...
        public static ICoreMessage Decode(IByteBuffer buffer)
        {
            ...
            {
                var total = buffer.ReadableBytes;
                var opCode = r.ReadEnum<ProudCoreOpCode>();
                var message = CoreMessageFactory.Default.GetMessage(opCode, r);

                if (PacketLog.Enabled)
                    PacketLog.WriteCore(true, opCode,
                        $"{message?.GetType().Name ?? "<sin handler>"} ({total} bytes, {r.BaseStream.Position} leidos)");
