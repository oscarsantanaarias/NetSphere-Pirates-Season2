...
namespace ProudNet
{
    ...
    internal static class PacketLog
    {
        ...
        private static readonly NLog.Logger Logger = NLog.LogManager.GetLogger("PacketLog");
        ...
        public static void Write(bool incoming, ushort opCode, string name)
        {
            ...
            Logger.Info($"{(incoming ? "REQ <-" : "ACK ->")} [{opCode}] {name}");
        }

        public static void WriteCore(bool incoming, object opCode, string name)
        {
            switch (opCode as string ?? opCode.ToString())
            {
                case "SpeedHackDetectorPing":
                case "UnreliablePing":
                case "Rmi":
                case "EncryptedReliable":
                    return;
            }

            lock (Sync)
            {
                Console.ForegroundColor = incoming ? ConsoleColor.DarkCyan : ConsoleColor.DarkGray;
                Console.WriteLine($"{(incoming ? "core <-" : "core ->")} [{opCode}] {name}");
                Console.ResetColor();
            }

            Logger.Info($"{(incoming ? "core <-" : "core ->")} [{opCode}] {name}");
        }
    }
    ...
    internal static class Constants
    {
        ...
        static Constants()
        {
            // .NET Core and later ship without the Windows code pages.
