...
namespace ProudNet
{
    public class ProudSession : IDisposable
    {
        ...
        // Season 2 wants the server's RSA public key inside the connection hint
        // and encrypts with it, so the pair has to outlive the handshake.
        internal System.Security.Cryptography.RSACryptoServiceProvider Rsa { get; set; }
        ...
        // Season 2 only reads plain Rmi from the server. The Auth service always
        // passed SendOptions.Reliable and worked; the game server relied on this
        // default and every one of its acks was dropped by the client.
