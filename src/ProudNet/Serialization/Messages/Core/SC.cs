...
namespace ProudNet.Serialization.Messages.Core
{
    ...
    // Season 2 wraps its traffic in this before the Rmi. Season 1 never sends
    // one, which is why it was never registered.
    [BlubContract]
    internal class UserMessageMessage : ICoreMessage
    {
        [BlubMember(0)]
        public uint Unk { get; set; }

        [BlubMember(1, typeof(ReadToEndSerializer))]
        public byte[] Data { get; set; }

        public UserMessageMessage()
        { }

        public UserMessageMessage(byte[] data)
        {
            Data = data;
        }
    }
