...
namespace ProudNet.Serialization.Messages.Core
{
    ...
    internal class NotifyServerConnectionHintMessage : ICoreMessage
    {
        ...
        // Season 2 reads a length-prefixed blob after the config and feeds it to
        // Proud::CRsaKey::FromBlob: it is the server's RSA public key, as a
        // Windows PUBLICKEYBLOB. Season 1 does not read this field at all.
        ...
        public NotifyServerConnectionHintMessage(NetConfigDto config)
            : this(config, Array.Empty<byte>())
        { }

        public NotifyServerConnectionHintMessage(NetConfigDto config, byte[] publicKey)
        {
            Config = config;
    ...
    internal class NotifyServerConnectSuccessMessage : ICoreMessage
    {
        ...
        [BlubMember(1, typeof(GuidSerializer))]
    ...
    {
        [BlubMember(0, typeof(GuidSerializer))]
