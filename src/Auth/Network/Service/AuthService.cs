...
namespace Netsphere.Network.Service
{
    internal class AuthService : ProudMessageHandler
    {
        ...
        [MessageHandler(typeof(CCheckOptionVersionReqMessage))]
        public Task CheckOptionVersionHandler(ProudSession session, CCheckOptionVersionReqMessage message)
        {
            Logger.Debug($"Option version {message.Version} for account {message.AccountId}");
            return session.SendAsync(new SOptionBinaryAckMessage(), SendOptions.Reliable);
        }
    }
}
