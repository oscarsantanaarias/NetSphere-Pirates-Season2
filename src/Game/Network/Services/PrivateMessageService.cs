...
namespace Netsphere.Network.Services
{
    internal class PrivateMessageService : ProudMessageHandler
    {
        ...
        [MessageHandler(typeof(CNoteReminderInfoReqMessage))]
        public async Task CNoteReminderInfoReq(ChatSession session)
        {
            await session.SendAsync(new SNoteReminderInfoAckMessage(
                (byte)session.Player.Mailbox.Count(mail => mail.IsNew), 0, 0))
