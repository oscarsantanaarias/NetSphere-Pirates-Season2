...
namespace Netsphere.Network.Services
{
    internal class AuthService : ProudMessageHandler
    {
...
#if CLIENT_1148
        private static readonly Version s_version = new Version(0, 8, 32, 31343);
#else
        private static readonly Version s_version = new Version(0, 8, 32, 45069);
#endif
        ...
        public async Task LoginHandler(GameSession session, CLoginReqMessage message)
        {
            ...
            {
                await session.SendAsync(new SDefaultNickAckMessage($"Player{account.Id}"))
                    .ConfigureAwait(false);
                return;
            }
        ...
        [MessageHandler(typeof(CCreateFirstCharacterReqMessage))]
        public async Task CreateFirstCharacterHandler(GameSession session, CCreateFirstCharacterReqMessage message)
        {
            if (session.Player == null || !string.IsNullOrWhiteSpace(session.Player.Account.Nickname))
            {
                session.CloseAsync();
            ...
            Logger.Info()
                .Account(session)
                .Message($"Creating first character with nickname {message.Nickname}")
                .Write();
            ...
            var granted = new List<GiveItemResultDto>();
            if (session.Player.CharacterManager.Count == 0)
            {
                var shop = GameServer.Instance.ResourceCache.GetShop();
                Logger.Info()
                    .Account(session)
                    .Message($"First character costumes: {string.Join(", ", message.Costumes)}")
                    .Write();

                var gender = CharacterGender.Male;
                foreach (var costume in message.Costumes)
                {
                    var shopItem = shop.GetItem(new ItemNumber(costume));
                    if (shopItem == null || shopItem.Gender == Gender.None)
                        continue;

                    gender = shopItem.Gender == Gender.Female ? CharacterGender.Female : CharacterGender.Male;
                    break;
                }

                try
                {
                    session.Player.CharacterManager.CreateFirst(0, gender);
                    session.Player.CharacterManager.Select(0);
                }
                ...
                foreach (var costume in message.Costumes)
                {
                    ...
                    try
                    {
                        var item = session.Player.Inventory.CreateSilent(costume, 0, 1);
                        session.Player.CharacterManager.CurrentCharacter.Costumes.Equip(item, (CostumeSlot)item.ItemNumber.SubCategory);
                        granted.Add(new GiveItemResultDto(costume));
                    }
                    catch (ArgumentException ex)
                    {
                        Logger.Warn($"Could not grant the starting costume {costume}: {ex.Message}");
                    }
                }
            }

            await session.SendAsync(new SGiveItemResultAckMessage(granted.ToArray()))
                .ConfigureAwait(false);
        ...
        private static async Task LoginAsync(GameSession session)
        {
            ...
            // Season 2 has no 59905: its random shop acks live at 60101-60105.
            // Sending it here hands the client an opcode it cannot resolve.
        ...
        public async Task Relay_LoginHandler(RelayServer server, RelaySession session, Message.Relay.CRequestLoginMessage message)
        {
            ...
            var plr = GameServer.Instance.PlayerManager[message.AccountId];
            Console.WriteLine(
                $"[relay] login desde {ip} cuenta={message.AccountId} sala pedida={message.RoomLocation.RoomId} " +
                $"| jugador en el game={(plr == null ? "NO" : "si")} " +
                $"| sala actual={(plr?.Room == null ? "ninguna" : plr.Room.Id.ToString())}");
