...
namespace Netsphere
{
    internal class Inventory : IReadOnlyCollection<PlayerItem>
    {
        ...
        public PlayerItem CreateSilent(ItemNumber itemNumber, byte color, uint count)
        {
            var shop = GameServer.Instance.ResourceCache.GetShop();
            var shopItem = shop.GetItem(itemNumber);
            if (shopItem == null)
                throw new ArgumentException($"Item {(uint)itemNumber} not found");

            var shopItemInfo = shopItem.ItemInfos.FirstOrDefault();
            if (shopItemInfo == null)
                throw new ArgumentException($"Item {(uint)itemNumber} has no info");

            var price = shopItemInfo.PriceGroup.Prices.FirstOrDefault();
            if (price == null)
                throw new ArgumentException($"Item {(uint)itemNumber} has no price");

            var effect = shopItemInfo.EffectGroup.Effects.FirstOrDefault();
            var item = new PlayerItem(this, shopItemInfo, price, color, effect?.Effect ?? 0, DateTimeOffset.Now, count);
            _items.TryAdd(item.Id, item);
            return item;
        }
