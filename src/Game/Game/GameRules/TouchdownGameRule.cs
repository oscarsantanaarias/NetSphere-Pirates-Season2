    ...
    internal sealed class TouchdownGameRule : GameRuleBase
    {
        ...
        public override void Update(TimeSpan delta)
        {
                ...
                RoundTime >= TimeSpan.FromSeconds(5)) // Let the round run for at least 5 seconds - Fixes StartResult trigger on game start(race condition)
            {
                ...
                // Here I set to not end room with 1 player for testing purposes
                if (Room.Players.Count > 1 && min == 0 && !Room.Options.IsFriendly)
        ...
        private bool CanStartGame()
        {
            ...
            return teams.Where(team => team.Count > 0)
                .All(team => team.Players.Any(plr => plr.RoomInfo.IsReady || Room.Master == plr));
        }
