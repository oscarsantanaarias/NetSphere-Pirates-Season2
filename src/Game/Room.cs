...
namespace Netsphere
{
    internal class Room
    {
        ...
        public void Join(Player plr)
        {
            ...
            if (plr.Club != null)
                Broadcast(new SEnteredPlayerClubInfoAckMessage { Player = plr.Map<Player, PlayerClubInfoDto>() });

            var gameRule = GameRuleManager.GameRule;
            var enterInfo = new EnterRoomInfoDto
            {
                RoomId = Id,
                GameRule = (uint)Options.MatchKey.GameRule,
                MapId = Options.MatchKey.Map,
                PlayerLimit = (byte)Options.MatchKey.PlayerLimit,
            ...
            plr.Session.SendAsync(new SEnteredPlayerClubInfoListAckMessage
            {
                Players = _players.Values.Where(p => p.Club != null).Select(p => p.Map<Player, PlayerClubInfoDto>()).ToArray()
            });
        ...
        public void ChangeRules(ChangeRuleDto options)
        {
            ...
            if (speccount > options.SpectatorLimit)
            {
